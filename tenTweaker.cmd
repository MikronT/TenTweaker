...
set officeSetupISO=files\microsoftOfficeProfessionalPlus2016Setup.iso
...
set command=%errorLevel%
if "%command%" == "2" if "%officeSetupURL%" NEQ "%officeSetupAdditionalURL%" (
...
:gpeditMSCSetupMenu
set gpeditMSCSetup_packagesList=files\gpeditMSCSetup_packagesList.txt

set gpeditMSCSetup_gpeditFile=not exist
for /f "delims=" %%i in ('dir /a:-d /b "%winDir%\System32\gpedit.msc"') do if "%%i" == "gpedit.msc" set gpeditMSCSetup_gpeditFile=exist

call :logo
echo.^(^i^) gpedit.msc Setup Menu
echo.
echo.
echo.^(^>^) Choose action:
echo.    ^(1^) Setup ^(or repair^) gpedit.msc       %gpeditMSCSetup_gpeditFile%
echo.
echo.    ^(0^) Go back
echo.
echo.
